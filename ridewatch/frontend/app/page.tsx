...
export default function Home() {
  ...
  async function start(event: FormEvent<HTMLFormElement>) {
    ...
    try { const ride = await createRide({ start_lat: location.latitude, start_lng: location.longitude, destination: destination.label, destination_lat: destination.latitude, destination_lng: destination.longitude, expected_distance_km: route.estimate.distance_km, expected_duration_minutes: route.estimate.duration_minutes, fare_estimate_id: fare.estimate.estimate_id, vehicle_number: normalizeVehicle(vehicle) || null }); router.push(`/ride/${ride.id}`); }
