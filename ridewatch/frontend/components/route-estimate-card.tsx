...
export default function RouteEstimateCard({ estimate, loading, error, retry, fare }: {
  ...
  return <section className="card" aria-live="polite">
    ...
    {estimate && <><dl className="mb-5 grid grid-cols-2 gap-5"><div><dt className="text-sm text-stone-500">Expected distance</dt><dd className="mt-2 text-2xl font-bold">{formatDistance(estimate.distance_km)} <span className="text-base font-normal">km</span></dd></div><div><dt className="text-sm text-stone-500">{estimate.traffic_aware ? "Traffic-aware duration" : "Expected duration"}</dt><dd className="mt-2 text-2xl font-bold">{formatDuration(estimate.duration_minutes)}</dd></div></dl><p className="mb-5 text-xs leading-5 text-stone-500">Via Amazon Location Service. Car routing estimates may differ from an auto-rickshaw’s route. Travel time and distance are estimates, not guarantees.</p></>}
    ...
    {fare.loading ? <p role="status" className="text-sm text-stone-500">Calculating fare estimate…</p> : fare.error ? <div role="alert" className="text-sm text-red-700"><p>{fare.error}</p><button type="button" className="secondary mt-3" onClick={fare.retry}>Retry fare estimate</button></div> : <FareSummary estimate={fare.estimate}/> }
  </section>;
}
