...
class FareService:
    def __init__(self, provider: FareProvider, repository: FareReportRepository, rules: MatchingRules = MatchingRules(), area: SupportedArea = SupportedArea()):
        self.provider, self.repository, self.rules, self.area = provider, repository, rules, area
        self._quotes: dict[UUID, tuple[FareQuery, FareEstimate]] = {}
        self._lock = Lock()
    ...
    def estimate(self, query: FareQuery) -> FareEstimate:
        ...
        with self._lock:
            self._quotes[result.estimate_id] = (query, result)
        return result
    ...
    def snapshot(self, query: FareQuery, estimate_id: UUID | None) -> FareEstimate:
        ...
        with self._lock:
            saved = self._quotes.get(estimate_id)
        if saved is None or saved[0] != query:
